...
router.post('/cli/add', async (req, res) => {
  try {
    ...
    if (type === 'http') {
      cliArgs.push('-s', 'user', '--transport', 'http', name, url);
      // Add headers if provided
      Object.entries(headers).forEach(([key, value]) => {
        cliArgs.push('--header', `${key}: ${value}`);
      });
    } else if (type === 'sse') {
      cliArgs.push('-s', 'user', '--transport', 'sse', name, url);
      // Add headers if provided
      Object.entries(headers).forEach(([key, value]) => {
        cliArgs.push('--header', `${key}: ${value}`);
      });
    } else {
      // stdio (default): claude mcp add -s user <name> <command> [args...]
      cliArgs.push('-s', 'user', name);
