...
process.stdin.on('end', () => {
  try {
    ...
    const toolInfo = data.tool_input || {};
    ...
    const payload = {
      ...
      metadata: {
        toolName,
        toolParams: toolInfo,
        requiresConfirmation: isHighRiskTool(toolName),
    ...
    console.error('claude-tool-pending hook error:', err);
    process.exit(0); // Exit cleanly to not block Claude
  }
});

function isHighRiskTool(toolName) {
  // Tools that might require user confirmation
  const highRiskTools = [
    'bash',
    'edit',
    'write',
...
function sendToServer(payload) {
  ...
  req.on('error', (err) => {
    console.error('Failed to send tool pending:', err.message);
    process.exit(0);
  });
